...
def get_api_key():
    """从环境变量获取 Alibaba API Key"""
    # 优先从环境变量获取
    api_key = os.environ.get('ALIBABA_API_KEY')
    if api_key:
        return api_key
    
    # 备用：从 OpenClaw 配置文件获取（兼容旧配置）
    config_path = "/root/.openclaw/openclaw.json"
    if os.path.exists(config_path):
        with open(config_path, 'r') as f:
            config = json.load(f)
            # 尝试 alibaba provider
            api_key = config.get('models', {}).get('providers', {}).get('alibaba', {}).get('apiKey')
            if api_key and not api_key.startswith('__OPEN') and not api_key.startswith('${'):
                return api_key
    ...
    raise Exception("未找到 阿里云 Alibaba API Key，请设置 ALIBABA_API_KEY 环境变量")
