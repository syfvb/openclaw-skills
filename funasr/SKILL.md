...
Use **FunASR** when:
- **Chinese audio** (best accuracy, beats cloud)
- Privacy-sensitive content
- No internet or unreliable connection
- Batch processing many Chinese files

Use **DashScope** when:
- **English or non-Chinese audio** (FunASR English accuracy ~70%, DashScope ~95%)
- Quick one-off transcription
- Multi-language mixed content
- No local GPU/CPU resources

**实测对比 (2026-05-24)**:
| 内容 | FunASR准确度 | DashScope准确度 |
|------|-------------|----------------|
| 中文演讲 | ~98% ✅ | ~90% |
| 英文演讲(Jobs 1983) | ~70% ⚠️ | ~95% ✅ |

FunASR英文问题：首词丢失、单词错误(wore→war)、语义错误(or older→ordered)、截断
