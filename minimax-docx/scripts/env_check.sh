...
# --- Optional: dotnet-script (for .csx scripts) ---
if command -v dotnet-script &>/dev/null; then
    dscript_ver=$(dotnet-script --version 2>/dev/null | head -1 || echo "?")
    printf "[OK]      %-14s %s (csx scripts)\n" "dotnet-script" "$dscript_ver"
else
    printf "[WARN]    %-14s not found — .csx scripts won't run\n" "dotnet-script"
    WARNINGS=$((WARNINGS + 1))
    case "$OS" in
        macos)        echo "           Install: brew install dotnet-script" ;;
        linux|wsl)    echo "           Install: dotnet tool install -g dotnet-script" ;;
        windows-shell) echo "           Install: dotnet tool install -g dotnet-script" ;;
    esac
fi
