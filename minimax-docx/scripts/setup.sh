...
# --- dotnet-script Installation (Optional, for .csx scripts) ---
install_dotnet_script() {
    step "Checking dotnet-script (optional: C# script execution)"

    if command -v dotnet-script &>/dev/null; then
        log "dotnet-script $(dotnet-script --version 2>/dev/null | head -1) already installed"
        return 0
    fi

    info "Installing dotnet-script (global .NET tool)..."
    dotnet tool install -g dotnet-script 2>>"$LOG_FILE" || {
        warn "dotnet-script installation failed (optional, .csx scripts unavailable)"
        warn "Install manually: dotnet tool install -g dotnet-script"
        warn "Note: requires .NET SDK 8.0+"
        return 0
    }

    # Add to PATH if not already there
    if [ -d "$HOME/.dotnet/tools" ] && ! echo "$PATH" | grep -q "$HOME/.dotnet/tools"; then
        info "Adding .dotnet/tools to PATH..."
        export PATH="$HOME/.dotnet/tools:$PATH"
        echo 'export PATH="$HOME/.dotnet/tools:$PATH"' >> "$HOME/.bashrc"
    fi

    if command -v dotnet-script &>/dev/null; then
        log "dotnet-script installed"
    else
        warn "dotnet-script not found after install (may need to restart terminal)"
    fi
}
...
print_summary() {
    ...
    echo "  dotnet-script: $(dotnet-script --version 2>/dev/null | head -1 || echo 'not installed (optional)')"
